...
def process_team(data: dict[str, Any]):
    ...
    # Prof. Oses's awards come from the CV (src/data/awards.json, generated by the
    # s4e feed from honors.json). "awards_extra" holds anything the site shows that
    # has no honors.json record; it is merged in ahead of the generated list.
    awards = data.get("awards") or []
    for member in data["team"].values():
        if set(member.get("titles", [])) & set(groups[0]["positions"]):
            member["awards"] = sorted(member.pop("awards_extra", []) + awards,
                                      key=lambda a: str(a.get("year")), reverse=True)
